...
**风物执守 · XY 操盘系统**（对外英文/GitHub 名 **XY Skill**）——一套装进 Claude Code / Codex / 豆包等 Agent 里的商业与私域操盘顾问。作者是商业博主"小爷"，本人有十几年、多次从 0 到过亿的私域操盘经验。这个项目把他的实战判断沉淀成 **46 个可直接调用的 Skill** 和 **27,966 条知识原子**，每个判断都带原子编号可查。
...
- **`知识库/`**（86 个文件）—— 这次蒸馏第三方咨询/管理类材料时，每个技能每条信条的中间产出草稿。留着当审计留痕，不必读，除非你要复核某条引用当初是怎么蒸馏出来的。
- **`docs/decisions/`**（1 个文件，2026-08-25 的技能盘点与方法论蒸馏决策记录（本地私有目录，不入库，文件名以本地为准））—— **这个建议你读一遍**，是这几天工作的完整决策记录，按时间顺序记了技能盘点、蒸馏计划、执行结果、发现的问题和修复过程，11 个编号章节。比这份交接文档细得多，遇到"为什么当初这么做"的问题，先查这份。
...
2. 咨询/管理类方法论改写库（adapted_consulting，14,421 条）蒸馏进 20 个技能正文，1,619+ 条引用，全部核对过零漏引、零来源泄漏。
...
2. **来源脱敏是硬红线**：知识原子重写后可以引用编号，但正文绝对不能点名任何第三方公司、咨询机构、书名、账号名——这条同样适用于本文档和仓库里所有公开文件。改动前先 grep 一遍确认没有泄露，改完再 grep 一遍确认。
...
有问题先查 `docs/decisions/` 下 2026-08-25 的技能盘点与方法论蒸馏决策记录，那份记得比这里细。这份交接文档写完之后，如果又发生了什么这里没提到的事，麻烦你自己在这份文档里续写一段，方便再往下交接。
