...
> 面向老板与创业者的商业中文 AI Skills 工具箱。四条主线：**私域与流量**（定位、选品、内容、导流、成交）、**商业与团队**（模式、合规、人效）、**IP 与内容创作**、**本体论（Ontology）与 FDE（Forward Deployed Engineer，前置部署工程师）方法论**——把生意里任何一段的真实问题交给 Agent，获得有依据的判断和马上能做的下一步。
>
...
小爷做私域十几年，从 0 带过好几个项目盘到过亿。风物执守把这些年真正管用的判断，拆成一套能查、能引用、能被验证的系统：

| | |
| --- | --- |
| 知识原子 | **27,966** 条，每条带来源和置信度，能独立成立、能互相回链 |
| 概念节点 | **333** 个，原子聚合后的结构化判断，覆盖 15 个话题 |
| Skill 数 | **46** 个，覆盖 **14** 个方向，装完就能用命令点名 |

[上手三步](#上手三步) · [能解决什么](#能解决什么) · [本体论与FDE怎么跑出来的](#本体论ontology与-fde-怎么跑出来的) · [装好之后有什么](#装好之后有什么) · [怎么装](#怎么装) · [指令清单](docs/XY指令清单.md)
...
| 公司内部规则一团乱、上了 AI 还是不好用 | 本体论/FDE：先讲清楚是什么、值不值得建，真要落地走闸门式交付流程 |
...
## 本体论（Ontology）与 FDE 怎么跑出来的

本体论（Ontology）是一套给"业务里的实体、类型、层级、关系"定规矩的方法，源头是哲学里研究"存在"的分支，后来被计算机科学借去，专门解决"数据堆成一坨、AI 看不懂业务结构"的问题；企业里干这件事的岗位叫 FDE（Forward Deployed Engineer，前置部署工程师）——带着工具住进客户的系统里，把客户自己的业务结构建出来，而不是卖一套通用模板。

XY 操盘系统没有停在"讲讲概念"：六步建模法（定边界→抽实体→定类型→建层级→抽关系→验证）真跑了两遍——一次在自己 27,966 条知识原子上，产出 333 个概念节点；一次在一个真实客户系统上做验证。跑完不直接发布，而是找另一个独立 AI 拿真实数据逐句核对结论，核对出的错误判断改完才重新发布——第六步"验证"要的就是这个，不是走个过场。

![本体论与 FDE 方法论](docs/assets/ontology-fde-flow.png)

这条线拆成两层，边界写死不含糊：

- **`xy-ontology`（免费）**：回答"本体论/FDE 是什么、跟知识图谱和 RAG 数仓有什么区别、值不值得给自己公司建"——负责先建立判断力，不接具体执行。
- **`xy-fde`（企业顾问入口）**：标品还是定制、怎么报价、组织内部推进阻力怎么应对——负责判断"这单接不接、怎么接"，真要对着客户自己的数据走完整建模流程，是另外的驻场服务。
- **定位边界**：这条线用在内部知识、运营、合规这类场景，不是"AI 帮你获客涨粉"的工具——本体论投入周期长，用在拉新上性价比不高，别用错地方。

## 装好之后有什么

46 个 Skill 覆盖 14 个方向，说人话就能自动进对门，也能直接用命令点名：
...
`knowledge/atoms.jsonl` 里 27,966 条知识原子，按 15 个话题、94 个细分块打了标签，条条能独立成立、能互相回链。来源分四块：小爷自己的实战（课程体系、真实账号和直播复盘、十几年创业史，带真实数字的都标 `high` 置信）；商业管理谈判这类方法论（全部重写成生意场景的话再入库）；抖音小红书微信视频号四大平台的现行规则；第三方付费训练营的方法论内容（讲者与学员姓名已脱敏，具体案例段落排除在外，只保留可复用的判断和做法）。检索排序上，自有实战永远排前面。
...
作者：**小爷** · 商业博主 · 私域操盘手 · 本体论/FDE方法论实践者 · [抖音](https://v.douyin.com/njWgCcCFUYY/)
