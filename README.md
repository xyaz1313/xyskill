...
>
> 本体论/FDE 这条线不是讲 PPT：六步建模法真跑在自己近 2.8 万条知识原子和一个真实客户系统上，跑出 **333 个概念节点**，还找了另一个独立 AI 逐句核对结果、挑出真错误改了才发布——过程和结论全部公开，见 [`xy-ontology`](skills/xy-ontology/SKILL.md)。
...
| | |
| --- | --- |
| 知识原子 | **28,226** 条，每条带来源和置信度，能独立成立、能互相回链 |
...
XY 操盘系统没有停在"讲讲概念"：六步建模法（定边界→抽实体→定类型→建层级→抽关系→验证）真跑了两遍——一次在自己近 2.8 万条知识原子上，产出 333 个概念节点；一次在一个真实客户系统上做验证。跑完不直接发布，而是找另一个独立 AI 拿真实数据逐句核对结论，核对出的错误判断改完才重新发布——第六步"验证"要的就是这个，不是走个过场。
...
`knowledge/atoms.jsonl` 里 28,226 条知识原子，按 15 个话题、94 个细分块打了标签，条条能独立成立、能互相回链。

**两个数字，先说清楚：**

- **按条数算，86% 来自外部方法论**——咨询框架、商业管理和谈判类的书、别人的课，全部重写成生意场景的话再入库，库里没有一段原文；每条的 `source_type` 都标明了这一点，不藏。有读者用 AI 算过这个比例，算对了。
- **按你拿到的答案算，94% 是小爷自己的实战和课程**——20 个典型问题跑一遍，前 5 条里 94% 来自真实账号、直播复盘、课程体系和十几年创业史（带真实数字的都标 `high` 置信）。检索排序永远把自家实战排前面，外部方法论是背景层，不是答案。

两个数字都能自己算：`python3 scripts/source-composition.py`。它同时给出库存构成、概念节点的证据构成、检索结果构成，谁跑结果都一样。

其余来源：抖音小红书微信视频号四大平台的现行规则（`platform_rule`）；对标账号内容（已脱敏）；第三方付费训练营的方法论（讲者与学员姓名已脱敏，具体案例段落排除在外，只保留可复用的判断和做法）。来源类型的完整定义在 `knowledge/ontology-schema.md`"来源类型"一节。
