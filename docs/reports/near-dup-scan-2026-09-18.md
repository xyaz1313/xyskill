...
## 执行结果（2026-09-18 收官审查第二轮）

A/C/N 三档 123 对逐对人工级判定（决定文件 `near-dup-decisions-2026-09-18.jsonl`），另加本体论章节两次原子化 7 对（`near-dup-decisions-ont-2026-09-18.jsonl`）与判读中新发现 3 对（`near-dup-decisions-extra-2026-09-18.jsonl`）。`scripts/atoms-apply-dedup.py` 执行：合并 91 对（删 91 条原子，入链改指、出链随迁、概念证据改指、16 个 skill 的 references 子集同步）、加 8 条 refines、19 对保留。D 档 120 对未判（弱重叠，留作下一轮线索）。执行中发现两份决定文件在 2 对上方向相反形成环，脚本已加"合并环"硬拦截。
